...
import { ProjectWorkspaceBar } from "@/components/jobsite/ProjectWorkspaceBar";
import { ScaleTestSurfaces } from "@/components/jobsite/ScaleTestSurfaces";
...
/**
 * Desktop Jobsite shell — fidelity freeze (`desktop/fidelity-v1`).
 * Behavior-identical extraction from the pre-split JobsiteApp layout.
 * Do not regress multi-window, dense nav, or portfolio workspace here for mobile.
 * @see docs/DESKTOP-FIDELITY.md
 */
export function DesktopJobsiteShell() {
  const view = useJobsiteStore((s) => s.view);
  const setView = useJobsiteStore((s) => s.setView);

  // Deep-link: /jobsite?view=map (and other views)
  useEffect(() => {
    if (typeof window === "undefined") return;
    try {
      const q = new URLSearchParams(window.location.search).get("view");
      const allowed: JobsiteView[] = [
        "feed",
        "report",
        "messages",
        "inspections",
      ...
      if (q && allowed.includes(q as JobsiteView)) {
        setView(q as JobsiteView);
      }
    } catch {
      /* ignore */
    }
  ...
  return (
    <div data-edition="desktop" className="contents">
    <AppShell active="jobsite" mobileNav={<JobsiteBottomNav />}>
      <main
        className={cn(
          "desktop-shell-inner mx-auto w-full px-3 py-4 sm:px-6 sm:py-8",
          view === "feed" ||
            view === "desk" ||
            view === "schedule" ||
            view === "contacts" ||
      ...
      >
        {/* Multi-project workspace: sync, lock, portfolio, cross-project lists */}
        <div className="mb-4">
          <ProjectWorkspaceBar />
        </div>
        {view === "report" ? (
...
function DesktopNav({ active }: { active: JobsiteView }) {
  ...
  return (
    <div
      className="hidden gap-1 rounded-2xl border border-[color-mix(in_oklab,var(--color-gold)_18%,var(--color-border))] bg-surface p-1 md:grid md:grid-cols-5 lg:grid-cols-9"
      role="tablist"
